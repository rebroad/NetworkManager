    ...
    case NM_SUPPLICANT_INTERFACE_STATE_COMPLETED:
        ...
        if (devstate == NM_DEVICE_STATE_CONFIG) {
            ...
            if (priv->roam_supplicant_wait_source) {
                ...
                nm_clear_g_source_inst(&priv->roam_supplicant_wait_source);
                nm_device_update_dynamic_ip_setup_on_roam(device, "roamed to a different AP");
            }
        }
        break;
    ...
    if (new_ap != priv->current_ap) {
        ...
        if (new_bssid) {
            ...
            if (nm_supplicant_interface_get_state(priv->sup_iface)
                ...
                nm_device_update_dynamic_ip_setup_on_roam(NM_DEVICE(self), "roamed to a different AP");
            } else {
